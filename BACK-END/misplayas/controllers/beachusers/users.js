...
async function listUsers(req, res, next) {
    ...
    try {
        ...
        let queryResults = await connection.query(
            `
        SELECT users.id, users.name, users.role, users.email, users.image, COUNT(reservations.id) AS Nºreservas, SUM(reservations.places) AS Nºplazas, MAX(reservations.date) AS ultima_reserva
FROM users
