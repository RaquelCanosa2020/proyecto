  ...
  try {
    ...
    if (email !== currentUser[0].email) {
      ...
      await connection.query(
        `
        UPDATE users 
        SET name=?, email=?, lastUpdate=UTC_TIMESTAMP, lastAuthUpdate=UTC_TIMESTAMP, active=false, registration_code=?, image=?
        WHERE id=?
      `,
        [name, email, registration_code, savedFileName, id]
      );
